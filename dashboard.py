...
# Firebase alert state tracking
alert_ref = db.reference('/alert_state')
alert_state = alert_ref.get()
...
if sensor_data:    
    ...
    # Send alert only once if MQ5 exceeds 100
    if mq5 > 100 and (not alert_state or not alert_state.get("alert_sent")):
        send_sms_alert("+919326735464", "Fire Detected. Stay Safe.")
        alert_ref.update({"alert_sent": True})

    # Reset alert state when back to safe
    if mq5 <= 100 and alert_state and alert_state.get("alert_sent"):
        alert_ref.update({"alert_sent": False})
